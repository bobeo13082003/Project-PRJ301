...
public class RemoveCartServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String txt = "";
        if (arrCookies != null) {
            for (Cookie arrCooky : arrCookies) {
                if (arrCooky.getName().equals("cart" + acc.getId())) {
                    txt += arrCooky.getValue();
                    arrCooky.setMaxAge(0);
                    response.addCookie(arrCooky);
                }
            }
        }
        ...
        if(!out.isEmpty()){
            Cookie cookie = new Cookie("cart" + acc.getId(), out);
